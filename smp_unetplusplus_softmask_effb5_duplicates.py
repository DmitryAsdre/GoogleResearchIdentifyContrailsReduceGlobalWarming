...
class CFG:
    exp_name = 'softmask_efficientnet_b6_512_unet++_bce_8_31_augs_02_duplicates'
    ...
    TH = 0.038
    DUPLICAtES_PATH = '../data/duplicates_0.05_64.parquet'
    ...
    train_augmentations = A.Compose([A.RandomRotate90(p=0.2),
                                    A.Flip(p=0.2)])
...
class ContrailDataset(Dataset):
    ...
    def __init__(self, dataset_path, data_type, duplicates_path=None, th=CFG.TH, train_augmentations=None):        
        ...
        if self.duplicates_path:
            self.duplicates = pd.read_parquet(self.duplicates_path)
            self.duplicates = self.duplicates[self.duplicates.distance < th]
            self.duplicates_names = set(self.duplicates.name_1.tolist())
    ...
    def __getitem__(self, idx):
        ...
        if self.data_type == 'train':
            if self.duplicates_path and record_id in self.duplicates_names:
                human_pixel_mask = np.load(os.path.join(record_dir, 'human_individual_masks.npy'))
                human_pixel_masks = [human_pixel_mask]
                for record_id_ in self.duplicates[self.duplicates.name_1 == record_id].name_2:
                    record_dir = os.path.join(self.dataset_path, record_id_)
                    cur_human_pixel_mask = np.load(os.path.join(record_dir, 'human_individual_masks.npy'))
                    human_pixel_masks.append(cur_human_pixel_mask)
                human_pixel_mask = np.concatenate(human_pixel_masks, axis=3).mean(axis=3)
            else:
...
if __name__ == '__main__':    
    ...
    train_ds = ContrailDataset(CFG.DATATRAIN_PATH, 'train', train_augmentations=CFG.train_augmentations, th=CFG.TH, duplicates_path=CFG.DUPLICAtES_PATH)
    ...
    model = smp.UnetPlusPlus(encoder_name=CFG.backbone, decoder_attention_type=CFG.decoder_attention_type, decoder_use_batchnorm=CFG.decoder_use_batchnorm).to(CFG.device)
