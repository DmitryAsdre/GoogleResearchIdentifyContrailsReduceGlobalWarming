...
class CFG:
    exp_name = 'softmask_efficientnet_b7_256_unet++_bce_8_31_augs_5_cleaned_dataset'
    ...
    CLEAN_DATA_PATH = '../data/duplicates_0.05_64.parquet'
...
class ContrailDataset(Dataset):
    ...
    def __init__(self, dataset_path, data_type, train_augmentations=None):        
        ...
        if self.data_type == 'train':
            df_clean = pd.read_parquet(CFG.CLEAN_DATA_PATH)
            df_clean = df_clean[df_clean.distance < 0.04]
            names_to_clean = set(df_clean.name_1.tolist() + df_clean.name_2.tolist())
            print(len(names_to_clean), len(self.imgs))
            self.imgs = list(set(self.imgs) - names_to_clean)
            print(len(self.imgs))
